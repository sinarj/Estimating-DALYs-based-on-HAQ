##########################################################
# Description: Create HAQ index 2016 for the US
# (age group specific). This is done separately cause
# the US is not a most detailed location, but we want the
# final predictions based on the regressions for the US
##########################################################
...
amenabledf_2017<-fread(paste0(data_dir,"combined_data/global_data" ,"/US_data_noncancer.csv"))
...
amenabledf_2017_MI_ratios <-fread(paste0(data_dir,"combined_data/global_data" ,"/US_data_cancer.csv"))
...
minmaxdf<-fread(paste0(data_dir, "/combined_data/min_max/minmax_noncancer_v2.csv"))
...
minmaxdf_MI <- fread(paste0(data_dir, "/combined_data/min_max/minmax_cancer_v2.csv"))
...
weightsdf <- fread(paste0(data_dir, "/weights/cause_weights_by_age_long_form_v2.csv"))
weightsdf$cause_id <- as.integer(weightsdf$cause_id)

indexdf <- inner_join(indexdf,
                      weightsdf[!is.na(cause_weight), c("cause_id","age_group_id", "cause_weight")],
...
index2016_all <-indexdf[year_id==2016][cause_id!=100]
write.csv(index2016_all, file = paste0(data_dir, "/results/haq_2016_bycauseage_USonly.csv"))
#also making a local copy on my drive
write.csv(index2016_all, file = paste0("/ihme/homes/arjuns13/notebooks/Documents/Data/haq_2016_bycauseage_USonly.csv"))

#now the aggregated cause with cause_id=100
index2016_agg <-indexdf[year_id==2016][cause_id==100]
write.csv(index2016_agg, file = paste0(data_dir, "/results/haq_2016_by_allcause_age_USonly.csv"))
#also making a local copy on my drive
write.csv(index2016_agg, file = paste0("/ihme/homes/arjuns13/notebooks/Documents/Data/haq_2016_by_allcause_age_USonly.csv"))
